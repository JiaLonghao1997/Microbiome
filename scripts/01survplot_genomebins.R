...
infile = file.path("F:\\Zhaolab2020\\virome\\TGScompare\\02genome_bins", "complete_genomes_modify.txt")
genomes = read.table(infile, sep='\t', header = TRUE, stringsAsFactors = FALSE)
print(unique(genomes$quality))
print(genomes$Sample)
high_genomes <- genomes[(genomes$quality=='complete') & (grep('A_', genomes['Sample'])), ]
levels(high_genomes$quality) <- c(levels(high_genomes$quality), 1) 
high_genomes$quality[high_genomes$quality=='complete']  <- 1 
...
ggsurvplot(fit, data = high_genomes, surv.scale='default', risk.table = TRUE,
           ...
           legend.title = "", # 设置图例标题
           legend.labs = c("mNGS", "PacBio","Nanopore","mNGS+PacBio", "mNGS+Nanopore"), 
