...
def data_preprocess(data, feature="function"):
    ...
    data_rel = (data.T/data.T.sum(axis=0)).T
    data = data_rel
    if feature == "function":
        func_cutoff = 1e-6
        func_pesudo = 1e-9
...
def train_cv(data,label,model,type='function',cv=10,repeat=10):
    """
    ...
    feature_data = pd.read_csv('data/testset_subchallenge2_files/'+data,sep='\t').T[1:]
    label = pd.read_csv('data/testset_subchallenge2_files/'+label,sep='\t').values[:,2]
    label =  np.array([1 if i != 'nonIBD' else -1 for i in label]).reshape(len(feature_data),1)
    #print(label)
...
if __name__ == '__main__':
    ...
    # print('\n\n')
    #train_cv('TrainingSchirmer_TaxonomyAbundance_matrix.txt', 'Class_labels_Schirmer.txt', model='randomforest',type='species')
    # print('\n\n')
